        ...
        <el-table style="width: 100%" :data="spu.spuSaleAttrList" border>
          ...
          <el-table-column prop="prop" label="属性值名称列表" width="width">
            <template slot-scope="{row}">
              <el-tag v-for="(tag,index) in row.spuSaleAttrValueList" :key="tag.id" :disable-transitions="false" closable @close="row.spuSaleAttrValueList.splice(index,1)">
          ...
          <el-table-column prop="prop" label="操作" width="width">
            <template slot-scope="{}">
              <el-button type="danger" icon="el-icon-delete" size="mini" circle @click="spu.spuSaleAttrList.splice($index,1)" />
            </template>
          </el-table-column>
        </el-table>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async addOrUpdateSpu() {
      ...
      const result = await this.$API.spu.reqAddOrUpdateSpu(this.spu) // 发请求
      if (result.code === 200) {
    ...
    async addSpuData(category3Id) {
      ...
      const tradeMarkResult = await this.$API.spu.reqTradeMarkList() // 获取品牌的信息
      if (tradeMarkResult.code === 200) {
        this.tradeMarkList = tradeMarkResult.data
      }
      const saleResult = await this.$API.spu.reqBaseSaleAttrList() // 获取平台全部的销售属性
      if (saleResult.code === 200) {
        this.saleAttrList = saleResult.data
      }
    },
